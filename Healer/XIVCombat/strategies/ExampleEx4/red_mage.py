...
def use_swift_cast(data: 'LogicData', is_must=False):
    return UseAbility(
        a('即刻咏唱'), data.me.id,
        ability_type=AbilityType.oGCD if is_must else AbilityType.GCD,
...
class RDMLogic(Strategy):
    ...
    def global_cool_down_ability(self, data: 'LogicData') -> AnyUse:
        ...
        if data.gauge.mana_stacks >= 3:
            ...
            ) else a('赤核爆'), aoe_target.id, wait_until=lambda: data.gauge.mana_stacks < 3)
        ...
        if prepare_cnt < 2 and res:
            ...
            if data.config['use_swift'] and not data[a('即刻咏唱')]:
                return use_swift_cast(data, True)
    ...
    def non_global_cool_down_ability(self, data: 'LogicData') -> AnyUse:
        ...
        if (
                ...
                (not (speed_swing.intersection(data.effects_set) or s('促进') in data.effects_set) or s('倍增') in data.effects or data.me.level < 78)
        ):
            if data.me.level < 78:
        ...
        if data[a('短兵相接')] < 5 and data.gcd > .7 and data.actor_distance_effective(single_target) <= data.config['jmp_distance']:
            return UseAbility(a('短兵相接'), single_target.id, rtn_period=.5)
    ...
    def global_cool_down_ability_on_count_down(self, data: 'LogicData') -> AnyUse:
        if data.last_count_down > 19: return
        effects = data.me.effects.get_set()
