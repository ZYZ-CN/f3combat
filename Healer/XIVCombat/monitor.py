...
class Actor:
    ...
    def __init__(self, aid: int, actor: 'ActorStruct|None' = None):
        ...
        self.death_cnt = 0
        self._owner_id = None
        self.last_record = [.0, .0]  # damage, heal
        self.first_record = [.0, .0]  # damage, heal

    def update_time(self, time: float, is_heal: bool):
        idx = int(is_heal)
        self.last_record[idx] = time
        if self.first_record[idx] == 0:
            self.first_record[idx] = time - 1
...
class Monitor:
    ...
    def _update_record_time(self, time: float, idx: int):
        self.last_record[idx] = time
        if self.first_record[idx] == 0:
            self.first_record[idx] = time - 1

    def update_record_time(self, time: float, is_heal: bool):
        self._update_record_time(time, int(is_heal))
        ...
        for target_id, effects in action_event.targets.items():
            ...
            if target_actor.actor and target_id > 0x40000000 and is_actor_status_can_damage(
                ...
                try:
                    ...
                    pass
        if time_update[0]: self.update_record_time(occur_time, False)
        if time_update[1]: self.update_record_time(occur_time, True)
        self.process_outdated_effects()
        return True

    def on_dot(self, occur_time: float, dot_event: 'DotEvent'):
        self.update_record_time(occur_time, False)
    ...
    def on_hot(self, occur_time: float, hot_event: 'HotEvent'):
        self.update_record_time(occur_time, True)
    ...
    def process_outdated_effects(self):
        while self.out_date_idx < len(self.effects) and self.effects[self.out_date_idx].occur_time < self.last_record_all - 60:
    ...
    def dps(self, actor_id: int):
        ...
        return actor.damage / (self.last_record[0] - actor.first_record[0]) if actor.first_record[0] else 0

    def dpsm(self, actor_id: int):
        actor = self.get_actor(actor_id)
        return actor.damage_in_min / min(self.last_record[0] - actor.first_record[0], 60) if actor.first_record[0] else 0

    def hps(self, actor_id: int):
        actor = self.get_actor(actor_id)
        return actor.heal / (self.last_record[1] - actor.first_record[1]) if actor.first_record[1] else 0

    def hpsm(self, actor_id: int):
        actor = self.get_actor(actor_id)
        return actor.heal_in_min / min(self.last_record[1] - actor.first_record[1], 60) if actor.first_record[1] else 0

    def dtps(self, actor_id: int):
        actor = self.get_actor(actor_id)
        if not actor: return 0
        return actor.taken_damage / (self.last_record[0] - actor.first_record[0]) if actor.first_record[0] else 0

    def dtpsm(self, actor_id: int):
        actor = self.get_actor(actor_id)
        if not actor: return 0
        return actor.taken_damage_in_min / min(self.last_record[0] - actor.first_record[0], 60) if actor.first_record[0] else 0

    def death(self, actor_id: int):
        return self.get_actor(actor_id).death_cnt

    def last_record_in(self, actor_ids, _type=0):
        if _type == 1: return max(self.get_actor(actor_id).last_record[0] for actor_id in actor_ids)
        if _type == 2: return max(self.get_actor(actor_id).last_record[1] for actor_id in actor_ids)
        return max(max(self.get_actor(actor_id).last_record) for actor_id in actor_ids)

    @property
    def last_record_all(self):
        return max(self.last_record)

    @property
    def first_record_all(self):
        return min(self.first_record)

    @property
    def last_damage_record(self):
        return self.last_record[0]

    @property
    def last_heal_record(self):
        return self.last_record[1]

    @property
    def first_damage_record(self):
        return self.first_record[0]

    @property
    def first_heal_record(self):
        return self.first_record[1]
