...
class XivCombat(PluginBase):
    ...
    if 1:
        ...
        def get_monitor_with_check(self, occur_time):
            m = self.get_monitor()
            if m.last_record_all and occur_time - m.last_record_all > split_time or m.zone_id != api.get_zone_id():
                m = self.new_monitor()
            return m
    ...
    if 1:
        ...
        def _layout_team_dps(self, last_time: float):
            ...
            m = self.get_monitor()
            return {
                'period': int(m.last_record_all - m.first_record_all),
                'zone': m.zone_id,
        ...
        def layout_team_dps(self):
            return self._layout_team_dps(self.get_monitor().last_record_all)
