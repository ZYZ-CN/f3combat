...
from FFxivPythonTrigger.meta_data import action_by_name, status_by_name
...
def a(action_name: str) -> int:
    return action_by_name[action_name.lower()].id


def s(status_name: str) -> int:
    return status_by_name[status_name.lower()].id


def cnt_enemy(data: 'LogicData', ability, default_target=None, valid_enemies=None):
    if default_target is None: default_target = data.target
    target, cnt = multi_enemy_selector.select(data, valid_enemies or data.valid_enemies, ability)
